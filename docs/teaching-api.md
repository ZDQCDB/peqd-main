...
**接口地址**: `POST /teaching/courses/{courseId}/checkin`

**功能描述**: 教师对指定课程进行签到，支持照片上传
...
**请求参数**: FormData
```
photo: File           // 签到照片文件（必填）
note: String          // 签到备注（可选）
location: String      // 签到位置（可选）
```
...
**接口地址**: `GET /teaching/teacher/courses`
...
**接口地址**: `GET /teaching/teacher/attendance`
...
- `school_admin`: 校级管理员，可查看全校数据
- `department_admin`: 院系管理员，只能查看本院系数据
...
CREATE TABLE courses (
  ...
  department_id VARCHAR(50) NOT NULL COMMENT '院系ID',
...
CREATE TABLE attendance_records (
  id VARCHAR(50) PRIMARY KEY,
  schedule_id VARCHAR(50) NOT NULL COMMENT '课程安排ID',
  teacher_id VARCHAR(50) NOT NULL COMMENT '教师ID',
  attendance_time TIMESTAMP COMMENT '签到时间',
  status ENUM('pending', 'completed', 'late') DEFAULT 'pending' COMMENT '签到状态',
...
### 签到照片表 (attendance_photos)
```sql
CREATE TABLE attendance_photos (
  id VARCHAR(50) PRIMARY KEY,
  attendance_id VARCHAR(50) NOT NULL COMMENT '签到记录ID',
...
CREATE TABLE departments (
  ...
  code VARCHAR(20) NOT NULL COMMENT '院系代码',
